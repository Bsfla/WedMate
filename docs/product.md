...
- 대분류 4개는 **추가·삭제하지 않는다** — 배분·차트 색·결산 구조가 4개에 묶여 있다. 이름 변경만 허용.
  추가는 DB가 막는다(`unique(couple_id, major_key)`에 쓸 다섯 번째 키가 없다), 삭제 금지는 앱 규칙이다.
- 중·소분류는 추가 / 이름변경 / 순서변경 / **보관(archive)** 가능.
- **삭제 대신 보관**한다. 지출이 이미 참조 중인 카테고리를 지우면 과거 기록이 깨진다.
  보관된 항목은 새 지출의 선택지에서 빠지되 기존 기록과 결산에는 남는다.
- **중분류를 보관하면 그 아래 소분류도 함께 보관된다. 해제도 대칭이다** (→ D-070).
  집계 View가 소분류의 보관 여부만 싣기 때문에, 부모만 꺼두면 예산·결산·빠른입력이 각자
  조상을 확인해야 하고 한 곳만 빠뜨려도 보관이 조용히 샌다.
- 순서변경은 **드래그가 아니라 ↑↓ 버튼**이다(→ D-020). 보관된 항목은 언제나 그룹 맨 뒤에 모이고
  화살표가 붙지 않는다(→ D-068).
- 행을 누르면 **이름 변경 + 보관** 시트가 열리고, 행 우측의 ↑↓만 인라인이다 (→ D-066).
  375px에서 44px 타깃 넷을 한 행에 넣으면 이름 칸이 100px 아래로 내려간다.
