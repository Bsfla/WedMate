...
data/
  ...
  list-row.tsx        href/onClick 추가 — 행 전체가 터치 타깃이 되고 › 표식이 붙는다.
                      ...
                      + trailingAction — 행 버튼 **바깥**의 조작부(↑↓). 버튼 안에 버튼을
                        넣을 수 없어서 구조로 막는다. `<li>`가 flex가 되고 행은 flex-1
...
- **대분류는 섹션 헤더**(색 점 + 이름 + `meta`로 "중 4 · 소 12"), 중분류는 그 안의 그룹 헤더
  (`bg-muted` · 48px · `text-body-sm font-bold text-muted-foreground`), 소분류만 `ListRow`.
...
**한 행의 44px 타깃은 둘까지다** (→ D-066). 소분류 행이 지는 동작은 이름변경·↑·↓·보관 넷인데
375px에서 44px 넷은 간격까지 164px을 먹어 이름에 100px도 남지 않는다.
→ **행 전체 = 편집 시트(이름 변경 + 보관)**, **행 우측 = ↑↓**. 순서변경만 인라인에 남기는
이유는 그것만이 **움직이는 것을 눈으로 봐야 하는 동작**이라서다.

**없는 화살표는 버튼만 지우고 자리는 남긴다** (→ D-067). 첫 행의 ↑, 마지막 행의 ↓는 D-061대로
렌더하지 않지만 `size-11` 빈 칸은 유지한다 — 자리까지 접으면 ↓의 x좌표가 행마다 달라져
연타가 빗나간다. 활성 형제가 1개뿐이면 열 전체를 그리지 않는다.

**행 단위 액션의 실패는 그 행 아래 `InlineError`다.** 트리에 행이 40개라 화면 상단 알림은
어느 행이 실패했는지 말하지 못한다. 액션 결과에 `categoryId`를 실어 **자기 행만** 그린다.
